...
| `deactivated` is reachable exactly once and never cleared. | The terminal guard before branch dispatch, plus the absence of any branch that writes the field back. | Nothing in the codebase; the guard runs before the `when` and every branch reproduces the datum wholesale. |
