...
- **`CHANGELOG.md` exists** at the repository root, carrying the semver policy (an equivalency *answer* changing, or a codebase being re-pinned, is MAJOR), the change types, the entry-style rules above, and the pre-release checklist. Adding a release entry means bumping each changed assessment's `## Document Version` and its mirrors.
