...
> The *implementation being approved* is
> [`cardano-foundation/cpt-rwa-ch-de-cmta-reference`](https://github.com/cardano-foundation/cpt-rwa-ch-de-cmta-reference)
> — "Programmable asset tokens on Cardano — German and Swiss profiles", the Aiken
> (Plutus V3) contracts under `validators/` and `lib/`, at commit **`ff5624e`** (2026-08-24).
> It is the **successor** to the `security-token` substandard of the CIP-113
> Programmable Tokens Platform (assessed in [`README-cip113-security-token.md`](./README-cip113-security-token.md)), which was
> itself a port of [`fn-bafin-cardano-sc`](./README-fn-bafin-cardano-sc.md).
> The repository ships **two profiles over one shared codebase**: a *German profile*
> (eWpG / BaFin) and a *Swiss profile* (CMTA Framework); only the metadata schema
> and which behaviours are mandatory differ.
...
> 2. **Internal adversarial review, not an audit.** An internal review of the compliance layer found eleven defects (two critical), all fixed and each pinned by a test in `validators/regression.ak` and written up in [`documents/security/security-fixes.md`](./cpt-rwa-ch-de-cmta-reference/documents/security/security-fixes.md).
...
|---|---|---|---|
| ID 14 — Deactivate contract | n | n (partial) | **y** — irreversible `DeactivateContract`, admin-signed, requires a paused protocol first |
...
| Standard burn on a frozen address | permitted (divergence) | permitted (divergence) | **not permitted** — matches CMTAT; must route through the seizure path |
| Burn while paused | permitted | permitted | **blocked in practice** by the CIP-113 base layer (see [Implementation Details](#implementation-details)) |
| Upgradeability | n | n (partial) | **partial** — swappable minting authority + registry-node upgrade + one-way `LockUpgrades` |
...
- Reference implementation compared against: **CMTAT v3.2.0** (Solidity) — the repository names it explicitly as its semantic reference.
...
| 14   | Deactivate contract | `deactivateContract`                 | Role-restricted (admin authorized)        | Must permanently disable the token (except in upgradeability patterns where deactivation behavior is explicitly defined). | y | **Admin** (`admin_credential_hash`, must sign) | `DeactivateContract` sets `deactivated = True`. Preconditions mirror CMTAT: the protocol must **already be paused**, the admin must sign, no security token may move in the same transaction, and the output datum must flip exactly that one field. **Irreversible by construction:** a terminal guard at the top of the spend validator (`expect !input_datum.deactivated`) rejects every subsequent spend of the GlobalState UTxO, before branch dispatch — so minting dies (`MintSecurity` can never run again), the pause can never be lifted, and no branch exists that could clear the flag. Transfer, seizure, the minting proxy, the minting authority, the denylist and the power-users list each additionally refuse to run once it is set. Supply is **not** required to be zero at deactivation, so the CMTA spec's *cancelled* (burn down first) and *immobilised* (freeze in place) variants are both supported. |
...
| Forced burn | `forcedBurn` | ✘ | ✔ | ✘ | ✘ | y | Negative mint through `minting_authority_validator`, gated on `can_burn`. No holder consent is needed, but — unlike in the two predecessor codebases — a burn is **not** exempt from the compliance gates: it spends a programmable-base UTxO, so the sender-side transfer logic runs over it. Retiring a sanctioned or uncooperative holder's position therefore requires routing through the seizure path as well, i.e. `can_burn` **and** `can_force_transfer`. |
