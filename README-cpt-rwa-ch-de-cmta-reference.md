...
| 14   | Deactivate contract | `deactivateContract`                 | Role-restricted (admin authorized)        | Must permanently disable the token (except in upgradeability patterns where deactivation behavior is explicitly defined). | y | **Admin** (`admin_credential_hash`, must sign) | `DeactivateContract` sets `deactivated = True`, admin-signed and reachable only from an already-paused protocol. Irreversible by construction, and supply need not be zero first. See the note below. |
